...
public class MoXlsxSaxAnalyser implements ExcelReadExecutor {
    ...
    public MoXlsxSaxAnalyser(MoXlsxReadContext xlsxReadContext, InputStream decryptedStream) throws Exception {
    ...
    private OPCPackage readOpcPackage(XlsxReadWorkbookHolder xlsxReadWorkbookHolder, InputStream decryptedStream)
        ...
        if (Boolean.TRUE.equals(xlsxReadWorkbookHolder.getMandatoryUseInputStream())) {
    ...
    public void execute() {
        for (ReadSheet readSheet : sheetList) {
            ...
            if (readSheet != null) {
                ...
                parseXmlSource(sheetMap.get(readSheet.getSheetNo()), new MoXlsxRowHandler(xlsxReadContext));
                // Read comments
